...
class ProxmoxHandler(CertificateHandler):
    def renew(self, cert_path: str, key_path: str) -> bool:
        ...
        try:
            nodes_resp = requests.get(f"{url_base}/nodes", headers=headers, verify=False, timeout=10)
    ...
    def provision_token(host, user, password, node='pve'):
        ...
        try:
            ...
            try:
                nodes_resp = requests.get(f"{base_url}/nodes", cookies=cookies, verify=False, timeout=10)
                if nodes_resp.status_code == 200:
