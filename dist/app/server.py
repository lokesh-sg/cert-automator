...
# --- Certificate Sources Management ---
@app.route('/api/sources', methods=['GET'])
def get_sources():
    if manager.is_locked:
        return jsonify({"success": False, "message": "System is locked"}), 401
    sources = manager.config_mgr.get_sources()
...
def add_source():
    try:
        data = request.json
        manager.config_mgr.add_source(data)
        return jsonify({"success": True, "message": "Certificate Source added successfully"})
    except Exception as e:
        return jsonify({"success": False, "message": str(e)}), 400
...
def update_source():
    try:
        ...
        if not source_id:
             return jsonify({"success": False, "message": "Source ID is required"}), 400
             
        manager.config_mgr.update_source(source_id, data)
        return jsonify({"success": True, "message": "Certificate Source updated successfully"})
    except Exception as e:
        return jsonify({"success": False, "message": str(e)}), 400
...
def delete_source():
    try:
        ...
        manager.config_mgr.delete_source(source_id)
        return jsonify({"success": True, "message": "Certificate Source deleted successfully"})
    except Exception as e:
        return jsonify({"success": False, "message": str(e)}), 400

@app.route('/api/sources/pull/<source_id>', methods=['POST'])
def pull_source(source_id):
    result = manager.pull_certificate_from_source(source_id)
    
    # Auto-Sync logic for manual UI triggers
    downstream_count = 0
    if result.get('success') and result.get('changed') and 'pack_name' in result:
        pack_name = result['pack_name']
        services = manager.get_services()
        for svc in services:
            pack = svc.get('cert_pack_id') or svc.get('cert_pack')
            if pack == pack_name and svc.get('enabled', True):
                name = svc.get('name')
                manager.logger.info(f"API Auto-Sync: Pack '{pack_name}' updated. Pushing to '{name}'...")
                manager.renew_service(name)
                downstream_count += 1
        
        if downstream_count > 0:
            result['message'] += f" and automatically pushed to {downstream_count} dependent service(s)!"
            result['downstream_updated'] = downstream_count
