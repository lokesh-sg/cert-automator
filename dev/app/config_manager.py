...
class ConfigManager:
    ...
    def load_config(self) -> dict:
        ...
        if not os.path.exists(self.config_path):
            self.config = {'services': [], 'certificate_sources': []}
        ...
        try:
            ...
            if self.crypto.is_encrypted(self.config_path):
                ...
                with open(self.config_path, 'r') as file:
                    self.config = yaml.safe_load(file) or {'services': [], 'certificate_sources': []}
    ...
    def get_sources(self) -> list:
        """
        Returns the list of certificate sources defined in the config.
        """
        return self.config.get('certificate_sources', [])
    ...
    def validate_source_data(self, data: dict):
        """Strict validation for certificate source data."""
        ...
        if data['type'] == 'npm':
            if not data.get('host') or not data.get('user') or not data.get('domain'):
                raise ValueError("NPM source requires host, user, and domain")
        elif data['type'] == 'acme':
            if not data.get('domain') or not data.get('dns_token'):
                raise ValueError("ACME source requires domain and DNS API token")
                
        return data

    def add_source(self, source_data):
        source_data = self.validate_source_data(source_data)
        sources = self.get_sources()
        if any(s.get('id') == source_data.get('id') for s in sources):
            raise ValueError("Source ID already exists")
            
        sources.append(source_data)
        self.config['certificate_sources'] = sources
        self.save_config()
        
    def update_source(self, source_id, new_data):
        if any(k in new_data for k in ['id', 'name', 'type']):
            sources = self.get_sources()
            current = next((s for s in sources if s.get('id') == source_id), None)
            if current:
                temp = current.copy()
                temp.update(new_data)
                self.validate_source_data(temp)
                
        sources = self.get_sources()
        found = False
        for i, s in enumerate(sources):
            if s.get('id') == source_id:
                sources[i].update(new_data)
                found = True
                break
